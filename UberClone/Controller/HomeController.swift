...
extension HomeController: CLLocationManagerDelegate {
    func locationManager(_ manager: CLLocationManager, didStartMonitoringFor region: CLRegion) {
        if region.identifier == AnnotationType.pickup.rawValue {
            print("DEBUG: started monitoring pickup region")
        }
        if region.identifier == AnnotationType.destination.rawValue {
            print("DEBUG: started monitoring destination region")
        }
    }
...
extension HomeController: RideActionViewDelegate {
    ...
    func uploadTrip(_ view: RideActionView) {
        ...
        PassengerService.shared.uploadTrip(pickupCoordinates, destinationCoordinates) { error, red in
            if let error = error {
                print("DEBUG: Failed to upload trip, \(error.localizedDescription)")
                return
            }
            UIView.animate(withDuration: 0.3) {
